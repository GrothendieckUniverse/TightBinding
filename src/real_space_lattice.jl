...
- fields:
    ...
    - `twisted_phases_over_2π::Vector{Float64}`: twisted phases φ/(2π) along each periodic direction. Non-zero values are only allowed where `pbc_indicator[d] == true`. Inserting fluxes is equivalent to shifting the crystal momentum grid by φ/L.
...
mutable struct Real_Space_Lattice{T}
    ...
    pbc_indicator::Vector{Bool} # whether to apply periodic boundary condition in direction-i
    twisted_phases_over_2π::Vector{Float64} # twisted phases φ/(2π); non-zero only where pbc[d]==true
...
function initialize_real_space_lattice(;
    ...
    for d in 1:dim
        if !pbc_indicator[d] && twisted_phases_over_2π[d] != 0.0
            error("Check input! Twisted phases are only allowed along periodic directions: twisted_phases_over_2π[$d] = $(twisted_phases_over_2π[d]) is non-zero, but pbc_indicator[$d] = false.")
        end
    end
